...
describe('ProtectedLayout', () => {
  ...
  it('redirects to onboarding when session exists but onboarding not complete', async () => {
    ...
    render(<ProtectedLayout />)
    await waitFor(() =>
      expect(mockRedirect).toHaveBeenCalledWith({ href: '/(onboarding)/goal' }),
    )
  })
...
export default function TargetsScreen() {
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.saveButton} onPress={handleSave} accessibilityLabel="Save targets">
